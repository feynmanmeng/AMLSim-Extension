...
def gen_data():
    ...
    for r in range(0, edges.shape[0]):
        ...
        src = edges['src'][r]
        dst = edges['dst'][r]
        amount = edges['amount'][r]
        step = edges['step'][r]
        alertid = edges['alertid'][r]
        issar = edges['issar'][r]
        per = [id, src, dst, amount, step, alertid, issar]
        lst_tran_export.append(per)
...
def create_transaction(tx, data):
    for id, src, dst, amount, step, alertid, issar in data:
        tx.run("match (a1:Account{id:$src}), (a2:Account{id:$dst})"
               "create (a1)-[:TRANS{id:$id, amount:$amount, step:$step, alertid:$alertid, issar:$issar}]->(a2)",
               id=int(id), src=int(src), dst=int(dst), amount=float(amount), step=int(step),
               alertid=int(alertid), issar=bool(issar)
               )
